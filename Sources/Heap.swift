...
struct Heap<Element: Equatable> {
    ...
    func getFirstIndex(of element: Element, startingAt startingIndex: Int = 0) -> Int? {
        ...
        let childIndicies = getChildIndices(ofParentAt: startingIndex)
        return getFirstIndex(of: element, startingAt: childIndicies.left)
            ?? getFirstIndex(of: element, startingAt: childIndicies.right)
    }
    ...
    func isHeap(sortedBy areSorted: @escaping (Element, Element) -> Bool) -> Bool {
        ...
        for parentIndex in stride(from: count / 2 - 1, through: 0, by: -1) {
            ...
            if rightChildIndex < count
                && areSorted(self[rightChildIndex], parent) {
                return false
            }
        }
